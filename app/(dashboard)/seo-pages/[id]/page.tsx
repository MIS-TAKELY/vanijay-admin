...
const GET_SEO_PAGE = gql`
  query GetSeoPage_Edit($id: String!) {
    seoPage(id: $id) {
      id
      urlPath
      metaTitle
...
const UPDATE_SEO_PAGE = gql`
  mutation UpdateSeoPage_Edit($id: String!, $input: UpdateSeoPageInput!) {
...
export default function EditSeoPagePage() {
    ...
    const { data: pageData, loading: pageLoading } = useQuery(GET_SEO_PAGE, {
        variables: { id },
        skip: !id
    });

    const { data: catData, loading: catLoading } = useQuery(GET_CATEGORIES);
    const [updateSeoPage, { loading: isUpdating }] = useMutation(UPDATE_SEO_PAGE, {
        onCompleted: () => {
    ...
    useEffect(() => {
        if (pageData?.seoPage) {
            const sp = pageData.seoPage;
            setMeta({
                urlPath: sp.urlPath || "",
                metaTitle: sp.metaTitle || "",
                metaDescription: sp.metaDescription || "",
                priceThreshold: sp.priceThreshold ? String(sp.priceThreshold) : "",
            });
    ...
    if (pageLoading) {
        return (
            <div className="flex flex-col items-center justify-center p-24 space-y-4 text-muted-foreground">
                ...
                <p className="animate-pulse">Loading endpoint data...</p>
            </div>
        );
    }
                ...
                <div className="flex items-start justify-between gap-4">
                    <div>
                        ...
                        <p className="text-muted-foreground mt-2 text-sm">
                            Modify category sections, pin products, and update routing logic.
                        </p>
                    </div>
